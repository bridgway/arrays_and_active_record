...
class ActiveRecordTest < Test::Unit::TestCase
  ...
  def test_select
    ...
    color_group = Car.group(:color => "cheetah").count
    assert_equal Car.where(:color => "cheetah").count, color_group
  end
  ...
  def test_like
    ...
    your_cars = Car.find("condition LIKE (?)",  "%good%")
  ...
  def test_limit
    limited_cars = nil
    cars = 5.times.map { Car.create }
    limited_cars = Car.last(4)
    assert_equal cars.offset(1).first(4), limited_cars
  end
  ...
  def test_offset
    offset_cars = nil
    cars = 5.times.map {Car.create}
    offset_cars = Car.last(5).limit(2)
